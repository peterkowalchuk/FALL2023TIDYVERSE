...
### cleaning
Im going to drop the columns with the na values
```{r}
## print names in df for copying pasting purposes
names(full_movie_ratings)
## store columns to delete in vector
columns_to_drop <- c("Film list here: https://www.boxofficemojo.com/year/2021/", "Year.y", "Foreign Gross ($million).x","Script Type.x", "Worldwide Gross ($million).x", "% of Gross earned abroad.x", "Oscar Winners.x" , "Oscar Detail.x", "None.x", "financial source, if not The numbers.x", "Domestic Gross.y", "Opening weekend ($million).y","Opening Weekend ($).y", "Genres.y", "Primary Genre.y", "Audience vs Critics deviance.x", "Foreign Gross.x", "Script Type.y", "Rotten Tomatoes  critics.y", "Metacritic  critics.y", "Average critics.y" )
